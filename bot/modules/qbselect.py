...
def select(update, context):
    ...
    if dl.status() not in [
        ...
        sendMessage(
            'Task should be in downloading status or in pause status incase message deleted \
               by wrong or in queued status incase you used torrent file!',
            context.bot,
            update.message)
        return
    ...
    try:
        ...
        sendMessage(
            "This task not started from qBittorrent",
            context.bot,
            update.message)
        return
    ...
    if WEB_PINCODE:
        ...
        buttons.buildbutton(
            "Select Files",
            f"{BASE_URL}/app/files/{hash_}?pin_code={pincode}")
...
select_handler = CommandHandler(BotCommands.QbSelectCommand, select, filters=(
