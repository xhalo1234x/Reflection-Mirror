...
from bot import AUTO_DELETE_UPLOAD_MESSAGE_DURATION, AUTO_DELETE_MESSAGE_DURATION, LOGGER, status_reply_dict, status_reply_dict_lock, \
...
def sendMarkup(text: str, bot, message: Message,
...
def auto_delete_upload_message(
        bot,
        cmd_message: Message,
        bot_message: Message):
    if cmd_message.chat.type == 'private':
        ...
        try:
            # Skip if None is passed meaning we don't want to delete bot or cmd
            # message
            deleteMessage(bot, cmd_message)
            deleteMessage(bot, bot_message)
        except AttributeError:
...
def update_all_messages(force=False):
    with status_reply_dict_lock:
        if not force and (
            not status_reply_dict or not Interval or time() -
            list(
                status_reply_dict.values())[0][1] < 3):
            return
        ...
        for chat_id in status_reply_dict:
            if status_reply_dict[chat_id] and msg != status_reply_dict[chat_id][0].text:
                if buttons == "":
                    ...
                    rmsg = editMessage(
                        msg, status_reply_dict[chat_id][0], buttons)
